...
def main():
    ...
    for project_name, project_path in repositories_dict.items():
        ...
        for class_name in sig_dataframe.class_name.unique().tolist():
            ...
            df = pd.concat([df, pd.DataFrame({'class_name': [class_name], 'class_context': [current_class.get_class_formatted()]})])
        ...
        most_likely_domain_class = detect_project_seed(custom_embedder, project_name, project_class_components)
        print(f"Most likely domain class: {most_likely_domain_class}\n")
        most_likely_domain = project_class_components[most_likely_domain_class]
        ...
        i = 1
        class_context = most_likely_domain.get_class_formatted()
        ...
        while len(project_class_components) > 0:
            ...
            if i == 1: 
                ...
                suggested_concept = CodePreprocessor.extract_suggestion(LLMHelper.generate_similar_concepts(class_context, llm, model, log_time))

            print(f"Starting iteration {i}...")
            # Should follow a particular template to ease comparison
            # TODO: Store the suggested concepts to compare with further iterations
            suggested_concept = CodePreprocessor.extract_suggestion(LLMHelper.generate_similar_concepts(class_context, llm, model, log_time))
            df['cosine_similarity'] = df['class_context'].apply(lambda class_definition: custom_embedder.calculate_similarity(class_definition, suggested_concept))
            ...
            with open(f"artifacts/{project_name}.txt", "a") as file:
                file.write(f"{found_class_object.get_class_formatted()}\n")


            # # Ask the LLM for auto evaluation
            # evaluation = LLMHelper.eval_suggestion(found_class_object.get_class_formatted(), llm, model, log_time)
            ...
            # if found_class_similarity > config['general']['domain_threshold'] or eval_classification == "domain":
            if found_class_similarity > config['general']['domain_threshold']:
                ...
                # TODO: Context a bit late here, should be added before the class is classified
                class_context += ",\n" + found_class_object.get_class_formatted()
            ...
            i += 1
            # log_df = pd.concat([log_df, pd.DataFrame({'list_of_given_concepts': [class_context], 'concepts_suggested_by_llm': [suggested_concept], 'closest_class': [found_class_name], 'cosine_similarity': [found_class_similarity], 'class_classification_similarity_based': [found_class_object.class_classification.value], 'llm_auto_classification': [eval_classification], 'll_auto_eval_score': [eval_score]})])
            log_df = pd.concat([log_df, pd.DataFrame({'list_of_given_concepts': [class_context], 'concepts_suggested_by_llm': [suggested_concept], 'closest_class': [found_class_name], 'cosine_similarity': [found_class_similarity], 'class_classification_similarity_based': [found_class_object.class_classification.value]})])
